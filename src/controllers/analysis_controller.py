...
from src.services.analysis_bucket_minio import (
    ANALYSIS_BUCKET,
    delete_session_analysis_data,
...
from src.services.summary_bucket_minio import (
    ...
    get_user_weekly_feedback_data,
    save_posture_insights,
)
...
class AnalysisController:
    ...
    def get_all_analyses(self, user_id):
        ...
        try:
            ...
            for analysis in analyses:
                ...
                analysis_dict["uploads"] = get_session_presigned_urls(
                    str(user_id), analysis.session_id
                )

                # Get presigned URLs for analysis JSON files
                analysis_dict["analysis_json_urls"] = (
                    self._get_session_analysis_json_urls(
                        str(user_id), analysis.session_id
                    )
                )

                # Get presigned URLs for feedback JSON files
                analysis_dict["feedback_json_url"] = (
                    self._get_session_feedback_json_url(
                        str(user_id), analysis.session_id
                    )
                )
    ...
    def get_analysis_by_id(self, user_id, analysis_id, is_admin):
        ...
        try:
            ...
            # Get presigned URLs for analysis JSON files
            analysis_dict["analysis_json_urls"] = self._get_session_analysis_json_urls(
                str(user_id), analysis.session_id
            )

            # Get presigned URLs for feedback JSON files
            analysis_dict["feedback_json_url"] = self._get_session_feedback_json_url(
                str(user_id), analysis.session_id
            )
    ...
    def _process_session_files_background(
        self, user_id, session_id, model_name, app, analysis_id
    ):
        ...
        try:
            ...
            with app.app_context():
                print(
                    f"Error in background processing for analysis {analysis_id}: {str(e)}"
                )
    ...
    def delete_analysis(self, user_id, analysis_id, is_admin=False):
        ...
        try:
            ...
            selected_analysis_user_id = (
                analysis.user_id
            )  # Use the actual user_id from the analysis record
            ...
            analysis_deletion_success = delete_session_analysis_data(
                str(selected_analysis_user_id), session_id
            )

            # Delete associated video files from MinIO videos bucket
            video_deletion_success = delete_session_video_files(
                str(selected_analysis_user_id), session_id
            )
                ...
                if date_generated:
                    try:
                        generated_date = datetime.fromisoformat(
                            date_generated.replace("Z", "+00:00")
                        ).replace(tzinfo=None)
                        ...
                        if generated_date.date() >= today.date():
                            presigned_url = get_posture_insights_presigned_url(
                                str(user_id)
                            )
                            return {
    ...
    def get_pdf_report(self, analysis: Analysis):
        """Get PDF report for a specific analysis"""
        try:
            pdf_url = get_pdf_report_url(analysis.user_id, analysis.session_id)
            ...
            try:
                ...
                response = minio_client.get_object("analysis-data", feedback_path)
                feedback_data = json.loads(response.read())

                pdf_bytes = generate_pdf_report(analysis, feedback_data)
                save_pdf_report(analysis.user_id, analysis.session_id, pdf_bytes)
                pdf_url = get_pdf_report_url(analysis.user_id, analysis.session_id)
...
def reattempt_analysis(analysis_id):
    ...
    result, status = analysis_controller.reattempt_analysis(
        user_id, analysis_id, model_name, current_user.is_admin
    )
    return jsonify(result), status
...
def get_analysis(analysis_id):
    ...
    result, status = analysis_controller.get_analysis_by_id(
        user_id, analysis_id, current_user.is_admin
    )
    return jsonify(result), status
...
def delete_analysis(analysis_id):
    ...
    result, status = analysis_controller.delete_analysis(
        user_id, analysis_id, current_user.is_admin
    )
    return jsonify(result), status
...
def get_pdf_report(analysis_id):
    ...
    if not analysis or (analysis.user_id != user_id and not current_user.is_admin):
        return jsonify({"error": "Analysis not found"}), 404

    result, status = analysis_controller.get_pdf_report(analysis)
    return jsonify(result), status
